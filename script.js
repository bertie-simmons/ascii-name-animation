...
function sampleNameIntoCells() {
    ...
    sampleCanvas.width = nameCols;
    sampleCanvas.height = nameRows;
    const sampleCtx = sampleCanvas.getContext("2d");
    sampleCtx.fillStyle = "#000";
    sampleCtx.fillRect(0, 0, nameCols, nameRows);
    sampleCtx.drawImage(nameImg, 0, 0, nameCols, nameRows);
    const { data } = sampleCtx.getImageData(0, 0, nameCols, nameRows);
    ...
    for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
            const inName =
                col >= startCol &&
                col < startCol + nameCols &&
                row >= startRow &&
                row < startRow + nameRows;
            let isLit = false,
                char = " ";
            if (inName) {
                const idx = ((row - startRow) * nameCols + (col - startCol)) * 4;
                const brightness =
